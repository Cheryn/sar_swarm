...
#    chromo_fit_based_conn = chr_fit_based_conn(chr_conn=chromo_conn)
    ...
    """chr_fit(chr_fit_based_dist=chromo_fit_based_dist,
...
def chr_best_fit_ind(chr_fit):
    ...
    elite_number = int(0.05*Config.pop_max)

    while len(chr_best_fit_index) < elite_number:
        # find the index of the highest fit
...
def chr_fit(chr_fit_based_dist):
    """
    ...
    for i in range(Config.pop_max):
        ...
        chr_fit[i][0] = chr_fit_based_dist[i][0]

    return chr_fit

# removed chr_fit_based_conn(chr_conn) because I dont see the point
...
def chr_pts_consecutive_dist(pop):
    ...
    chr_pop_dist = np.zeros((Config.pop_max, Config.chr_len))

    for i in range(Config.pop_max):

        for j in range(Config.chr_len-2):
